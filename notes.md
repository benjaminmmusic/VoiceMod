...
- **Pitch slider param-change smoothing — added 2026-05-25.** `PitchEffect` now ramps `_processor.PitchSemiTones` toward the slider's target value at `RampSemitonesPerBlock = 0.15f` semitones per capture block (about 7.5 sem/sec at 50 blocks/sec). Rationale: rapid integer-step writes to SoundTouch's pitch param caused audible discontinuities at block boundaries during slider drag. Ramp smooths the parameter change. Side effect: pitch *glides* to target rather than jumping — sounds slightly musical/slide-whistle-y during drag, settles to exact value after release. Acceptable. Slider also got `IsMoveToPointEnabled="True"` so click-on-track jumps to that position (snaps to nearest integer tick), useful for direct value testing.

- **Pitch ~+2 transit pop — deferred 2026-05-25.** A reproducible artifact remains: sliding through (or away from) pitch ~+2 produces an audible pop. Asymmetric — pitch -2 is clean. Tried and ruled out: smaller ramp (0.05), larger ramp (0.5), heavier overlap (25), much heavier tuning (100/30/15 caused constant popping). Even direct integer setter writes (pre-ramp) pop at +2, so it's not a fractional-transit artifact. Suspected SoundTouch internal heuristic threshold at the pitch ratio ~1.12 (= 2^(2/12)) but unverified without reading library source. Static values are clean — pop is motion-induced only. Deferred because chasing it cost real time with no resolution; better paths are (a) de-clicker post-process to mask transient discontinuities, or (b) algorithm swap to NWaves phase vocoder, which doesn't share SoundTouch's block-stitching artifact model. See improvements.md.
